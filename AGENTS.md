...
- **Atomic commits.** Each commit is a coherent unit with a descriptive message. Commits are checkpoints; you can roll back to any of them. For substantive completed repo work, commit automatically after the relevant verification passes unless the user explicitly asks not to commit, the work is intentionally exploratory, or a hard guardrail says to stop first.
- **Commit protocol.** Before committing, inspect `git status --short`, stage only the files that belong to the completed slice, and use a scoped message with `Why`, `Verification`, and `Boundary` body notes for substantive changes. After committing, verify the worktree state and report the commit hash.
