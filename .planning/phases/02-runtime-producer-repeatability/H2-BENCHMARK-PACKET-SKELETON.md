...
| `REVIEW-SPEC.md` | H2.S3 `/goal` agent | `cross-vendor-review` skill runner | Per `docs/REVIEW-SPEC-CONTRACT.md`. Declares `review_type: checkpoint`, declared output paths, write roots, retain-on-success flag. | `cross-vendor-review` runner preflight; `preflight.sh`. |
...
`SCOPES_REQUIRING_CROSS_MODEL = {"pass-claim", "main-merge", "broad-goal-restart"}` in `cbm/cli.py`. The H2.S3 checkpoint is scope `pass-claim`. The current dev-agent model family for H2.S1 and H2.S2 will likely be `claude` (Opus 4.7) or `gpt-5` (codex backend) depending on who runs each `/goal`. The H2.S3 reviewer must NOT be from the dev-agent's family. Same-model fallback is invalid for pass-claim scope per ADR-005. Reviewer-model-family checks fire both at the selector (`checkpoint_for_loop_scope` filters by declared scope) and at the gate (`SCOPES_REQUIRING_CROSS_MODEL` rejects current-family reviewers).
