...
- Reviewer model: a model family **not in** `current_dev_agent_model_families` from `cbm/loop_status_config.json` (currently `["gpt", "openai", "codex"]`). This is what `checkpoint_pass_claim_issues` (`cbm/cli.py:5795-5810`) actually enforces — `same_model_checkpoint` fires when `model_matches_family` substring-matches `reviewer_model_id` against the configured family list. **The check is against the RUNTIME-PRODUCER family (the backend running `cbm run` — codex-cli for H2), not the `/goal`-orchestrator family.** `cbm/loop_status_config.json:1` documents this explicitly: *"current_dev_agent_model_families lists the model families used as RUNTIME PRODUCERS (Surface Mapper, Skeptic, etc.) in cbm runs, not the build-agent that authored this repository's code."* For H2.S3 with `--backend codex-cli`:
  - `claude-opus-4-7` (or another Anthropic Claude family model) is a **valid** reviewer — `"claude-opus-4-7"` contains none of `gpt`/`openai`/`codex` substrings. This is the H1 precedent: the accepted H1 minimum-useful checkpoint at `.planning/reviews/2026-05-07-h1-minimum-useful-checkpoint/DISPOSITION.json` carries `reviewer_model_id: claude-opus-4-7` and currently passes `loop-status --scope pass-claim`.
  - `gpt-5` (or any model whose id contains a substring from the family list) **fails** the gate via `same_model_checkpoint`, even if the `/goal`-orchestrator is on Claude. Substring matching means `gpt-5`, `gpt-4`, `o1`, etc. are all blocked while the runtime-producer is codex-cli.
  - If the runtime-producer backend ever changes (e.g., a Claude Code backend lands per the ADR-006 / issue #23 abstraction work), `cbm/loop_status_config.json` should be updated to add `"claude"`/`"anthropic"` to the family list and BUILD-LOG.md should record the decision, per the config's `_documentation` guidance. Until that happens, the H2.S2/H2.S3 reviewer-family choice is "claude family OK; gpt/openai/codex family blocked." A future ADR (likely co-authored with ADR-006) should pin the broader "what reviewer-family is valid given a multi-backend producer ecosystem" question.
...
H2.S2 publication MUST preserve every `.research/run-h11-h2s2-<n>/` subtree the dispatch produced (logs, codex_outputs, ledgers, intermediates), copied into the packet under `.planning/benchmarks/<run-date>-h11-h2s2/.research/run-h11-h2s2-<n>/`. Under the future single-`cbm run` shape (post-ADR-006 / issue #23) there is one subtree (`-1`); under the H2.S2 brief's interim two-`cbm run` split (`GOAL-H2S2-LIVE-RUN.md` Phase 2) there are two subtrees (`-1` for Surface, `-2` for Skeptic); a retry that uses a different suffix preserves whatever the actual `--run-id` was. Treat this as a publication-completeness gate: the packet is not complete until every actual run-id subtree is preserved.
