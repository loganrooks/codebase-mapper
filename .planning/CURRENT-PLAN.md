...
Status: active; H1 minimum-useful close merged to main as `76db3bc` on 2026-05-16; H2.S1 deliverables produced on 2026-05-22 with H2 target locked as `h11`; next focus is H2.S2 live producer run in its own `/goal`
...
26. Draft the H2.S1 `/goal` brief that translates `.planning/HORIZONS.md` H2.S1 into an executable plan-production task. Status: completed. Brief drafted at `.planning/phases/02-runtime-producer-repeatability/GOAL-H2S1-REPEATABILITY-PLAN.md` on 2026-05-16; phase 02 PLAN/SUMMARY/VERIFICATION stubs created in the same PR per the `.planning/phases/<NN-slug>/` contract; pre-built H2-TARGET-SELECTION.md candidates landed in `0819bcd` on 2026-05-22; H2.S1 `/goal` executed on 2026-05-22 producing H2-PLAN.md (binding plan with concrete A1–A5 and absolute-path verification commands), H2-BENCHMARK-PACKET-SKELETON.md (H2.S2 + H2.S3 packet contracts), H2-PREFLIGHT.md (concerns 1–9), the locked-pick recording in H2-TARGET-SELECTION.md, and expansion of the phase 02 PLAN/SUMMARY/VERIFICATION stubs. H2 target locked as `python-hyper/h11` at SHA `62c5068c971579d61fa1b55373390e12f25fd856`, scope `h11/` package excluding `h11/tests/` (2,568 LOC across 11 source files; MIT). H2.S1 deliverables landed in this commit.

27. Open the H2.S2 `/goal` against a new `GOAL-H2S2-LIVE-RUN.md` authored against the locked target. H2.S2 runs the single-`cbm run` Surface Mapper + Skeptic against `python-hyper/h11@62c5068c` and produces the H2.S2 benchmark packet at `.planning/benchmarks/<run-date>-h11-h2s2/`. Status: pending. Per-slice `/goal` invocation: H2.S2 then H2.S3, each with its own `token_budget` and stop-and-surface boundary. Only H2.S3 carries the ADR-005 cross-model pass-claim checkpoint at scope `pass-claim`.

H1.S1 is completed narrowly. H1.S2a evidence-bundle repair is complete. H1.S2b real isolated Skeptic production is complete. H1.S2c challenge disposition and mapper response are complete. H1.S3 validated handoff and non-current-model checkpoint review are complete. H1 minimum-useful close (PR #1) is merged to main. H2.S1 is complete with deliverables landed at this commit; H2 target is locked. Phase B+ and repeatability claims remain blocked until H2.S2 produces live evidence and H2.S3 receives a non-current-model cross-vendor pass-claim checkpoint accept.

## Next `/goal` Track

The next `/goal` track is H2.S2 from `.planning/HORIZONS.md` and `.planning/phases/02-runtime-producer-repeatability/H2-PLAN.md`: run the live Surface Mapper + Skeptic against the locked H2 target.

H2.S1 is complete: target picked by the user (`python-hyper/h11` at SHA `62c5068c971579d61fa1b55373390e12f25fd856`, scope `h11/` package excl. tests), `H2-PLAN.md` filled with concrete A1–A5 and absolute-path verification commands, `H2-BENCHMARK-PACKET-SKELETON.md` enumerated, `H2-PREFLIGHT.md` documents concerns 1–9. H2.S2 `/goal` is now authored against the locked target; open it as the next slice.

Per-slice `/goal` invocation remains deliberate: H2.S2 (live Surface Mapper + Skeptic run on the chosen target) and H2.S3 (validated handoff + cross-vendor pass-claim checkpoint) each get their own `/goal`, their own `token_budget`, and their own stop-and-surface boundaries. Of these, only H2.S3 makes an H2 pass claim, so only H2.S3 carries the ADR-005 cross-model checkpoint at scope `pass-claim` (`SCOPES_REQUIRING_CROSS_MODEL` in `cbm/cli.py`). H2.S2 follows the standing per-slice cross-vendor review practice that produced H1.S1/H1.S2/H1.S2C reviews, but that slice-level review is not a pass-claim checkpoint in the ADR-005 sense — it does not gate `loop-status --scope pass-claim`. Bundling H2.S2 + H2.S3 into one mega-`/goal` would burn the H2.S2 budget envelope on the H2.S3 packet and would defer the only ADR-005-required checkpoint to the end of the mega-goal; the per-slice pattern mirrors the H1 execution shape (H1.S1 → H1.S2a → H1.S2b → H1.S2c → H1.S3) that produced the accepted H1 packet.

Current status: the guarded `codex-cli` backend produced one live smoke review artifact on the pinned MCP `src/git` benchmark, the live isolation probe reported no access to parent-only session context, a skill-loaded `skeptic@1.2` subprocess produced an ingested interpretive challenge on the same pinned benchmark, H1.S1 produced a real non-baseline `surface-mapper@1.2` surface map on MCP `src/git`, H1.S2b produced a real isolated `skeptic@1.2` review over that H1.S1 map, H1.S2c accepted the `auth-001` / `chl-10001` challenge as an alternative reading carried into handoff contestation, and H1.S3 prepared a handoff/checkpoint packet at `.planning/benchmarks/2026-05-07-mcp-git-h1s3-minimum-useful-handoff/` plus `.planning/reviews/2026-05-07-h1-minimum-useful-checkpoint/`. H2.S1 has now landed the binding H2 plan, packet skeleton, and preflight concerns under `.planning/phases/02-runtime-producer-repeatability/`, with H2 target locked as `python-hyper/h11@62c5068c971579d61fa1b55373390e12f25fd856`.

The H1 minimum-useful floor is accepted for one pinned external target. The next proof target is the H2.S2 live producer run against h11 (a `/goal` of its own), then H2.S3's validated handoff + cross-vendor pass-claim checkpoint.
