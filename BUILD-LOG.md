...
## 2026-05-16 — PR #14 CodeRabbit-finding remediation: phase 02 stubs + GOAL doc metadata

- Context: CodeRabbit reviewed PR #14 (the H2.S1 brief PR) and posted two actionable inline findings plus one pre-merge check failure. The findings cite the binding `.planning/**/*.md` AGENTS.md rules: planning docs must carry `Status` + `Last updated` + `Supersedes`/`Superseded by` metadata, and each `.planning/phases/<NN-slug>/` directory must contain `PLAN.md` + `SUMMARY.md` + `VERIFICATION.md` files. My initial decision to defer the phase 02 stubs as H2.S1 `/goal` deliverables violated the second rule — the contract treats those files as structural must-exist, not as deferred work. The user explicitly invoked the second-order audit feedback discipline (the `feedback_pr_review_discipline.md` memory in the auto-memory store outside the repo) for this remediation: do not just patch the called-out instances, look for the same class of issue elsewhere and audit cascade effects of each fix.
- Implemented:
  - **GOAL doc metadata**: added `Last updated: 2026-05-16` + `Supersedes: none` + `Superseded by: none` to the `GOAL-H2S1-REPEATABILITY-PLAN.md` frontmatter alongside the existing `Status: proposed` / `Date: 2026-05-16` fields.
  - **Phase 02 contract files**: created minimal stubs at `.planning/phases/02-runtime-producer-repeatability/PLAN.md`, `SUMMARY.md`, `VERIFICATION.md`. Each carries Status / Last updated / Supersedes / Superseded by metadata and a contract-honest body (PLAN: phase objective + H2.S1/S2/S3 track + stop conditions; SUMMARY: one orienting paragraph + state list; VERIFICATION: pending verification list with explicit "none yet for phase 02" completed-evidence statement). Stubs are intentionally minimal — the H2.S1 `/goal` expands them with concrete content as state advances.
  - **GOAL doc reconciliation**: updated the Deliverables section to "Produce (new) or expand (existing) these artifacts" with per-line annotations marking which exist vs which are new. Updated the "Planning Doc Update Requirements" section for PLAN/SUMMARY/VERIFICATION from "Create with this skeleton" to "Expand the stub" with concrete expansion guidance (bump `Last updated`, fill the track entries from `H2-PLAN.md`, copy verification commands). Updated the Completion Criteria from "Phase 02 directory contains PLAN/SUMMARY/VERIFICATION skeletons" to "stubs are expanded with concrete H2.S1 content".
- Second-order audit applied (per user instruction):
  - **Required Reading path verification**: bash-checked every path in the GOAL doc's Required Reading list (VISION.md, RUNTIME-CONSTITUTION.md, phase 01 docs, all referenced benchmarks, all ADRs, `schemas/*.schema.json`, `docs/review-playbook.md`, `.codex/skills/cross-vendor-review/SKILL.md`). All resolve. No hallucinated references in the brief.
  - **Same-class issue elsewhere**: H1.S3 / H1.S2 / H1.S2C GOAL docs (already on main as part of PR #1) carry only `Status:` + `Date:` and lack `Last updated:` / `Supersedes:` / `Superseded by:`. Same class of `.planning/**/*.md` metadata gap. Retroactive fix is OUT OF SCOPE for PR #14 because PR #14 is the H2.S1 brief PR; flagged to the user for a separate decision.
  - **Top-of-file metadata on other planning docs touched by this branch**: `CURRENT-PLAN.md`, `STATE.md`, `HORIZONS.md` already carry full metadata after this branch's earlier commits. No further action needed.
  - **BUILD-LOG.md scope**: at repo root, not under `.planning/`, so the `.planning/**/*.md` metadata rule does NOT apply. No frontmatter change needed.
  - **Cascade effects of stub creation**:
    - The BUILD-LOG.md slice entry for commit 1 says "Phase 02 PLAN.md/SUMMARY.md/VERIFICATION.md skeletons intentionally NOT pre-created" — that statement was true when that commit landed; this follow-up slice documents the revised decision rather than rewriting history.
    - STATE.md verification footer for the prior slice said the same — same handling.
    - The GOAL doc's Deliverables / Planning Doc Update Requirements / Completion Criteria sections all referenced the stubs as deferred work; all three updated in lockstep to refer to existing stubs.
  - **PR template vs CodeRabbit conflict**: `.github/PULL_REQUEST_TEMPLATE.md` uses `Summary / Focus / Test plan` section labels; CodeRabbit's pre-merge check requires `Why: / Verification: / Boundary:` labeled sections. PR #13 had `Summary / Test plan / Boundary` without explicit `Why:` / `Verification:` labels and was not blocked; the check was either added or strictened since then. PR #14's description is being rewritten with explicit `Why:` and `Verification:` labels to satisfy the check; the template-vs-check conflict is flagged to the user as a separate decision (update template OR relax check).
  - **No overclaim language**: re-read the three new stubs for "complete", "ready", "production-ready" type unbacked claims. None present. SUMMARY.md states "H1 acceptance preserved exactly as merged in `76db3bc`" which is backed by the commit hash; VERIFICATION.md says "None yet for phase 02" which is an explicit non-claim.
- Tests: none. Planning-doc-only iteration; no source change.
- Verification:
  - Pre-edit `git status` showed clean tree on `plan/h2s1-repeatability-brief`; post-edit shows the 3 new stub files + edits to the GOAL doc + BUILD-LOG + STATE.md.
  - No `pytest`, no `cbm-loop-status` run for this iteration (no source mutation; loop-status status is preserved from PR #13's verification footer).
- Boundary:
  - PR-#14-finding remediation; no horizon advance.
  - The stub bodies are minimal by design — the H2.S1 `/goal` is still responsible for the substantive content. The stubs satisfy the AGENTS.md `.planning/phases/<NN-slug>/` files-required contract; they do not pre-empt the `/goal`'s work.
  - H1.S3 / H1.S2 / H1.S2C GOAL docs on main retain the same metadata gap as before; their fix is a separate decision.

## 2026-05-17 — PR #14 second review round: ADR-005 scope, stale "skeletons" wording, broken anchors, PR #21 status, verification ordering

- Context: After the first PR #14 remediation commit (70fc56a), CodeRabbit posted two more inline findings (broken in-file fragment link in STATE.md:265 and empty-anchor markdown link in BUILD-LOG.md:1993) and Codex posted four findings (verification-sequence dirty_authority_docs gotcha; CURRENT-PLAN.md item 24 stale on PR #21; CURRENT-PLAN.md line 67 over-claiming ADR-005 per slice; HORIZONS.md still naming the phase 02 stubs as "to be produced"; PLAN.md objective over-claiming "carried challenge"; CURRENT-PLAN.md "Implementation phase bundle" still pointing at phase 01). The user explicitly invoked the three-axis review discipline: (1) generalization sweep, (2) second-order audit, (3) wider-horizon negotiation.
- Triage (11 findings total across both CR rounds and both Codex rounds):
  - CR-1 (GOAL doc metadata): already addressed in 70fc56a; no-op reply.
  - CR-2 / Codex CX-1 (phase 02 stubs missing): already addressed in 70fc56a; no-op reply.
  - Codex CX-2 (loop-status vs dirty_authority_docs): accept-with-revisions — split Required Verification into pre-commit (pytest + git diff --check) and post-commit (loop-status) sections in the GOAL brief, explain the AUTHORITY_DOC_PATHS interaction.
  - Codex CX-3 (PR #21 staleness in CURRENT-PLAN.md item 24): accept — STATE.md line 38 said "PR #21 merged + v1 fast-forwarded + stub reverted in PR #12 as bf277dc"; CURRENT-PLAN.md item 24 said "awaiting user review approval"; updated item 24 to mirror STATE.md.
  - Codex CX-4 (ADR-005 conflation): accept — per ADR-005 the cross-model gate fires on `SCOPES_REQUIRING_CROSS_MODEL = {"pass-claim", "main-merge", "broad-goal-restart"}`; only H2.S3 makes the H2 pass claim; H2.S1 and H2.S2 use per-slice cross-vendor reviews but those are not pass-claim checkpoints in the ADR-005 sense; reworded CURRENT-PLAN.md:67, STATE.md:39, STATE.md:108 in lockstep.
  - CR-3 / CR-4 (broken/empty anchor links): accept — replaced `[feedback memory](`#feedback-pr-review-discipline`)` in STATE.md:265 and `[feedback memory](#)` in BUILD-LOG.md:1993 with plain text plus pointer to the memory file in the auto-memory store outside the repo.
  - Codex CX-5 (HORIZONS.md still says "produce skeletons"): accept — same class as the 70fc56a GOAL-doc reconciliation; reworded HORIZONS.md:248, STATE.md:108, STATE.md:263, CURRENT-PLAN.md:59 in lockstep (4 instances of the same stale phrasing across authority docs).
  - Codex CX-6 (PLAN.md objective over-claims "carried challenge"): accept — reworded the objective to allow either a carried challenge or a "no-challenge" Skeptic result per H2.A2 and HORIZONS.md "non-trivial claim/challenge" wording.
  - Codex CX-7 (CURRENT-PLAN.md "Implementation phase bundle" stale): accept — updated to point at phase 02 (active) and noted phase 01 is closed/preserved.
- Class-of-bug sweeps performed proactively (axis 1):
  - Empty/broken markdown anchors: `grep -rn '](#'` across `.planning/ BUILD-LOG.md AGENTS.md VISION.md docs/` found only STATE.md:265 and BUILD-LOG.md:1993 — both flagged by CR; no others.
  - Stale "PLAN/SUMMARY/VERIFICATION skeletons" wording: found 4 instances (HORIZONS.md:248, STATE.md:108, STATE.md:263, CURRENT-PLAN.md:59) — Codex flagged 1 (HORIZONS.md:248); the other 3 were sweep finds.
  - ADR-005 per-slice conflation: found 3 instances (CURRENT-PLAN.md:67, STATE.md:39, STATE.md:108) — Codex flagged 1 (CURRENT-PLAN.md:67); the other 2 were sweep finds.
  - PR #21 staleness: found 1 instance (CURRENT-PLAN.md:57); STATE.md:38 was already current.
  - "Implementation phase bundle" pointer: found 1 instance (CURRENT-PLAN.md:16); STATE.md authority section refers to phase bundles generically (plural), so no update needed there.
  - H1 GOAL doc metadata gap (same class as CR-1): confirmed `.planning/phases/01-first-runtime-producer-evidence/GOAL-H1S2-EVIDENCE-BUNDLE-REPAIR.md`, `GOAL-H1S2C-CHALLENGE-DISPOSITION.md`, and `GOAL-H1S3-MINIMUM-USEFUL-HANDOFF.md` all carry only `Status:` + `Date:` and lack `Last updated:` / `Supersedes:` / `Superseded by:`. Out of scope for PR #14 (those are merged H1 evidence on main); flagged in the previous slice entry; remains a separate decision.
- Second-order effects audited (axis 2):
  - CURRENT-PLAN.md item 24 reword cross-references STATE.md:38 — both now say PR #21 merged + v1 fast-forwarded + stub reverted in PR #12 as `bf277dc`. Consistent.
  - "Implementation phase bundle" pointer is read by humans, not by `cbm-loop-status` (loop-status parses Current horizon / Current stage fields, not the bundle pointer); changing this is doc-only and does not break any programmatic check.
  - ADR-005 reword preserves the `SCOPES_REQUIRING_CROSS_MODEL` mechanical gate at H2.S3; the per-slice cross-vendor review practice (used at H1.S1/H1.S2*) continues for H2.S1/S2 but is now labeled as not-ADR-005 to avoid implying pass-claim gating where none exists.
  - Required Verification reword (pre-commit vs post-commit) is consistent with the H1.S3 brief's pattern in practice (verification was done post-commit there too; the H1.S3 brief just did not call out the dirty_authority_docs interaction explicitly). The new GOAL-doc wording makes the implicit pattern explicit.
  - PLAN.md objective reword preserves H2.A2's "challenge or no-challenge result" contract and HORIZONS.md "non-trivial claim/challenge" wording; the "non-trivial claim" path is now explicit so a high-quality map with no Skeptic challenge cannot be mistaken for an H2 failure.
- Wider-horizon negotiation (axis 3): no fix conflicts with ADR-005, ADR-001, ADR-003, RUNTIME-CONSTITUTION, or VISION. The verification-sequence reword does push subtly toward "commit before final verification", which is consistent with `cbm/cli.py`'s `AUTHORITY_DOC_PATHS` design but worth recording as a deliberate clarification, not a hidden change of practice.
- Implemented:
  - `.planning/CURRENT-PLAN.md`: bumped `Last updated:` to 2026-05-17; updated line 16 to point at phase 02 bundle (active); updated item 24 to mark PR #21 merged + stub reverted; reworded item 26 to refer to "expand existing stubs"; reworded the Next /goal Track section to restrict ADR-005 cross-model checkpoint to H2.S3 only.
  - `.planning/STATE.md`: bumped `Last updated:` to 2026-05-17; reworded line 39 to restrict ADR-005 to H2.S3; reworded line 108 to "expand existing stubs" and to restrict ADR-005 to H2.S3; reworded line 263 to note the deferred-stubs decision was revised in PR #14; replaced the broken fragment link on line 265 with plain text.
  - `.planning/HORIZONS.md`: bumped `Last updated:` to 2026-05-17; reworded line 248 to "expand existing PLAN/SUMMARY/VERIFICATION stubs".
  - `.planning/phases/02-runtime-producer-repeatability/PLAN.md`: bumped `Last updated:` to 2026-05-17; reworded the Objective to allow "carried challenge OR no-challenge" per H2.A2.
  - `.planning/phases/02-runtime-producer-repeatability/GOAL-H2S1-REPEATABILITY-PLAN.md`: bumped `Last updated:` to 2026-05-17; split Required Verification into pre-commit and post-commit sections with explicit `dirty_authority_docs` context.
  - `BUILD-LOG.md`: replaced the empty `[feedback memory](#)` anchor with plain text + a pointer to the auto-memory file outside the repo; this slice entry appended.
- Tests: none. Planning-doc-only iteration; no source change.
- Verification:
  - Pre-edit `grep -n 'each with their own ADR-005' .planning/STATE.md .planning/CURRENT-PLAN.md .planning/HORIZONS.md` showed 3 conflation hits; post-edit confirms 0.
  - Pre-edit `grep -n 'PLAN/SUMMARY/VERIFICATION skeletons' ...` showed 4 stale hits; post-edit confirms 0 remaining "produce skeletons" framings.
  - `grep -rn '](#' .planning BUILD-LOG.md` post-edit shows 0 empty/broken anchor links in the touched files.
  - No `pytest`, no `cbm-loop-status` run for this iteration (no source mutation; loop-status status preserved from PR #13's verification footer at 146 passed, 2 warnings).
- Boundary:
  - PR-#14 review-response only; no horizon advance, no code change.
  - The H1 GOAL doc metadata gap remains unfixed (still flagged for a separate decision).
  - The PR-template vs CodeRabbit-check conflict on `Why: / Verification: / Boundary:` labels remains unfixed (separate decision).
  - The H2.S1 `/goal` running on `plan/h2s1-repeatability-execution` is unaffected — H2-TARGET-SELECTION.md remains untracked there; the reworded brief is a clarification, not a contract change. The /goal can pick up the new wording on resume.

## 2026-05-17 — PR #14 third Codex round: residual ADR-005 site + verification-record loop + item-26 duplicate + relative-path validation + stale pass-claim prediction

- Context: Codex reviewed commit 418ca41 and posted 5 new inline findings, all P2. CR independently auto-confirmed the prior round was "addressed" on 4 of its threads (no new CR findings).
- Findings + disposition:
  - CX-R3-1 (GOAL doc:452 residual "each slice gets its own cross-model checkpoint review"): accept — same class as the CX-R4 ADR-005 conflation I swept in 418ca41, but I missed the closing paragraph of the brief itself. Reworded line 452 to restrict ADR-005 to H2.S3 and to label H2.S1/S2 as cross-vendor review per the standing practice.
  - CX-R3-2 (GOAL doc:369 verification-record loop): accept-with-revisions — the 418ca41 fix said "run post-commit loop-status, record outputs in STATE.md and BUILD-LOG.md". Recording the outputs creates a fresh dirty authority doc edit, which fails the next loop-status. Rewrote the Required Verification section to specify a two-commit pattern: Commit 1 lands H2.S1 with templated loop-status placeholders, post-commit runs loop-status, Commit 2 is an audit-only follow-up that replaces placeholders with actuals (no further loop-status to break the recursion).
  - CX-R3-3 (GOAL doc:310 "Add Active Recovery Sequence item 26"): accept — item 26 already exists in CURRENT-PLAN.md. Reworded to "Update item 26 status to 'completed'; if a separate entry is needed for H2.S2 dispatch, append item 27 (next unused number)".
  - CX-R3-4 (GOAL doc:203-207 relative-path validation): accept — `command_validate` (`cbm/cli.py:2391-2404`) does `if not path.is_absolute(): path = repo / path`, so `validate <relative-artifact> --repo <target-repo>` looks for the artifact under the target checkout, not the CBM repo. H1.S3's VERIFY.md:15 explicitly documents this exact FileNotFoundError. Reworded the template to require absolute paths for artifacts; added an inline comment explaining the resolution rule and citing the H1.S3 evidence.
  - CX-R3-5 (GOAL doc:375 stale pass-claim "expected to fail" prediction): accept — `loop-status --scope pass-claim` currently returns `status: ok` on the accepted H1 minimum-useful checkpoint (verified by direct CLI invocation). Removed the prediction; added a brief note clarifying that pass-claim is currently satisfied by H1 and that H2.A5 requires the new H2.S3 checkpoint to become the most-recent accepted pass-claim checkpoint before the H2 pass claim is supported. Also removed `--scope pass-claim` from the H2.S1 Required Verification command list (it isn't there, but the Expected Behavior section mentioned it as if it were).
- Class-of-bug sweeps performed proactively (axis 1):
  - "each ... cross-model checkpoint review" / "own cross-model checkpoint" in current authority docs: 1 hit at GOAL-doc:452 (this round's CX-R3-1) — the CURRENT-PLAN.md:67 and STATE.md:39 instances were already corrected in 418ca41 and remain correct. Post-edit sweep: 0 operational hits.
  - "Add Active Recovery Sequence item": 1 hit at GOAL-doc:310, no siblings in the brief.
  - relative-path validation commands: 5 sequential lines at GOAL-doc:203-207, all in the same homogeneous template block — all fixed in one edit. No other validation-command templates in the brief.
  - "pass-claim ... fail / block / still" predictions: 1 hit at GOAL-doc:375 (this round's CX-R3-5). HORIZONS.md:45 and STATE.md:283 also match the grep pattern but are H0-era historical records ("`pass-claim` remained blocked on same-model checkpoint, as intended" at H0.S1 verification time, before the H1 cross-model checkpoint landed). Those are accurate-as-of-then; leaving them alone preserves the audit trail.
- Second-order effects audited (axis 2):
  - The two-commit verification pattern at GOAL-doc:365-394 explicitly addresses the recursion break-point (do not run loop-status after Commit 2). The pattern reads as instruction, not as broken protocol invocation.
  - Absolute-path requirement at GOAL-doc:200-212 added an inline comment citing H1.S3 VERIFY.md:15 so the rationale stays attached when the brief is read in isolation.
  - The CX-R3-5 "pass-claim currently passes on H1" note is added at GOAL-doc:213-216 (the H2 verification commands section), and a parallel note is added at GOAL-doc:400 (the H2.S1 expected loop-status behavior section). Both notes link to the same fact: pass-claim is currently green on H1 evidence; H2.A5 requires the H2.S3 checkpoint to become the most-recent accepted one.
  - Item-26 update wording at GOAL-doc:310 names the "next unused number" pattern (item 27) for forward-compatibility, so the next /goal author doesn't repeat the duplication.
- Wider-horizon negotiation (axis 3): no fix conflicts with ADR-001, ADR-003, ADR-005, ADR-004, VISION.md, or RUNTIME-CONSTITUTION.md. The two-commit verification pattern is a clarification of the existing AGENTS.md "Commit protocol" guidance, not a policy change. The pass-claim clarification is consistent with the H1 acceptance boundary (the H1 minimum-useful pass-claim was accepted; H2 has its own pass-claim acceptance requirement at H2.S3).
- Implemented:
  - `.planning/phases/02-runtime-producer-repeatability/GOAL-H2S1-REPEATABILITY-PLAN.md`: reworded line 452 (ADR-005 scope); rewrote Required Verification section (365-394) with two-commit pattern; reworded line 310 (item 26 update vs add); replaced verification-command template at 203-207 with absolute-path requirement and inline rationale comment; replaced stale pass-claim prediction at 375 with current-state note; added pass-claim clarification at 213-216 and 400 (H2 acceptance template section and H2.S1 expected loop-status behavior).
- Tests: none. Doc-only iteration; no source change.
- Verification:
  - `grep -nE 'each.*cross-model|own.*cross-model' .planning/...` pre-edit returned 1 brief-doc hit (GOAL:452); post-edit returns 0 operational hits in the touched files.
...
## 2026-05-22 — H2.S1 plan production (`/goal` execution)

- Context: `/goal` invoked against `.planning/phases/02-runtime-producer-repeatability/GOAL-H2S1-REPEATABILITY-PLAN.md` on branch `plan/h2s1-repeatability-execution`. Pre-existing branch state included the H2.S1 brief (landed in PR #14 as `70fc56a..f6a0a58`), the phase 02 PLAN/SUMMARY/VERIFICATION stubs, and the pre-built `H2-TARGET-SELECTION.md` from `0819bcd` (three pinned-SHA candidates with h11 recommended). The brief's non-negotiable scope: produce the H2 repeatability plan, pick the second pinned target via user confirmation, define H2 acceptance and verification commands, prepare the benchmark packet skeleton — without launching any live Surface Mapper or Skeptic against the new target.
- Stop-and-surface user gate: presented the three pre-built candidates (h11 / mcp-filesystem / conc) via `AskUserQuestion` per the brief's "Do not pick the target unilaterally" mandate. User confirmed Candidate 2 (`python-hyper/h11`, scope `h11/` package excluding `h11/tests/`).
- Probe re-verification (read-only; no Surface Mapper, no Skeptic, no live producer):
  - `git ls-remote https://github.com/python-hyper/h11.git | grep 62c5068c971579d61fa1b55373390e12f25fd856` returned `HEAD` and `refs/heads/master`.
  - `git clone --filter=blob:none --no-checkout https://github.com/python-hyper/h11.git /var/tmp/cbm-h2-target-probes-20260522/h11` + `git checkout 62c5068c...` + `git rev-parse HEAD` returned the pinned SHA.
  - `git ls-files h11 | grep -E '\.(py|pyi)$' | grep -v '^h11/tests/' | xargs wc -l | tail -1` returned `2568 total` (matches pre-built probe at `0819bcd`).
  - 11 source files in scope: `__init__.py`, `_abnf.py`, `_connection.py`, `_events.py`, `_headers.py`, `_readers.py`, `_receivebuffer.py`, `_state.py`, `_util.py`, `_version.py`, `_writers.py`.
  - `head -4 LICENSE.txt` confirmed MIT (`The MIT License (MIT)` / `Copyright (c) 2016 Nathaniel J. Smith ...`).
- Schema-drift check for Concern 4 (Schema And Validator Drift) in the brief's preflight: `git log 76db3bc..HEAD --oneline -- schemas cbm/schemas` returned no commits; `git diff --stat 76db3bc..HEAD -- schemas cbm/schemas` returned empty. Schemas are byte-identical with `main` at the H1 merge commit. No migration needed for H2.
- H1 budget actuals captured for Concern 2 (Backend Budget And Cost): read from preserved H1 `run-manifest.json` files:
  - H1.S1 `run-mcp-git-surface-mapper-h1s1-6`: `codex-cli-skill-surface-map` step started `2026-05-02T20:49:51Z`, completed `2026-05-02T20:53:41Z` (3m50s wall clock); `gpt-5.4-mini` / `model_reasoning_effort=medium`; `--codex-timeout 600`; final manifest status `succeeded`. Used ~38% of the 10-minute timeout envelope.
  - H1.S2b `run-mcp-git-h1s2b-skeptic-1`: `codex-cli-skill-skeptic-review` step started `2026-05-07T12:33:54Z`, completed `2026-05-07T12:37:28Z` (3m34s wall clock); `gpt-5.4-mini` / `model_reasoning_effort=high`; same isolation flags + `--codex-timeout 600`; final manifest status `succeeded`. Used ~36% of the 10-minute timeout envelope.
  - Proposed H2.S2 envelope: same `gpt-5.4-mini` + 600s timeout + identical isolation flags. Soft estimate ~4–6m wall clock per producer for h11 (comparable target size to H1's MCP `src/git`). Hard abort condition: timeout-hit → `interrupted` status → stop and root-cause.
- Implemented:
  - New `.planning/phases/02-runtime-producer-repeatability/H2-PLAN.md`: binding H2 plan with concrete A1–A5 fields, absolute-path H2 verification command list (with the H1.S3 VERIFY.md:15 relative-vs-absolute path failure-mode warning), H2.S2 + H2.S3 slice structure, explicit rejection of a pre-emptive H2.S2a evidence-bundle-repair slice (rationale recorded), H1 caveat carryover classification.
  - New `.planning/phases/02-runtime-producer-repeatability/H2-BENCHMARK-PACKET-SKELETON.md`: H2.S2 packet contents + per-artifact contracts (producer / consumer / schema / validation command); H2.S3 handoff packet contents + contracts; H2.S3 cross-vendor pass-claim checkpoint review packet under `.planning/reviews/<run-date>-h2-repeatability-checkpoint/` per `.codex/skills/cross-vendor-review/SKILL.md`; ADR-005 cross-model requirement reiterated.
  - New `.planning/phases/02-runtime-producer-repeatability/H2-PREFLIGHT.md`: nine preflight concerns (1 language fit; 2 backend budget + H1 actuals; 3 cross-vendor review skill repeatability + modes + failure modes; 4 schema drift; 5 H1-caveat carryover; 6 protocol-state-machine authority surface; 7 public-API boundary and re-exports; 8 H2.A4 diff-record discipline; 9 `.research/` tree preservation).
  - Updated `.planning/phases/02-runtime-producer-repeatability/H2-TARGET-SELECTION.md`: added the "Locked Pick" section with the user-confirmed target identification + 2026-05-22 re-verification probe ledger; preserved the candidate ledger and rejected-pallets/click probe section as the input record to the decision.
  - Expanded `.planning/phases/02-runtime-producer-repeatability/PLAN.md`: bumped `Last updated:` to 2026-05-22; updated H2.S1 track entry to "completed" with concrete deliverables list; populated H2.S2 and H2.S3 entries with concrete target + packet paths + contract pointer; replaced Verification template prose with concrete H2.S1 verification expectations.
  - Expanded `.planning/phases/02-runtime-producer-repeatability/SUMMARY.md`: bumped `Last updated:` to 2026-05-22; updated state summary to record H2.S1 deliverables produced and H2 target locked; preserved H1 acceptance preservation note.
  - Expanded `.planning/phases/02-runtime-producer-repeatability/VERIFICATION.md`: bumped `Last updated:` to 2026-05-22; added "Completed Verification" section pointer to STATE.md verification footer; reworded "Pending Verification" to reflect H2.S1 completion.
  - Updated `.planning/CURRENT-PLAN.md`: bumped `Last updated:` to 2026-05-22; updated top-of-file Status to record H2.S1 deliverables landed; updated item 26 Status to "completed" with concrete deliverable list and locked target; added item 27 (open H2.S2 `/goal` against locked target); reworded `Next /goal Track` to point at H2.S2 (h11) and to reflect H2.S1 completion; reworded `Current status:` to record H2.S1 close.
  - Updated `.planning/HORIZONS.md`: bumped `Last updated:` to 2026-05-22; reworded H2.S1 status to "current; plan prepared" with concrete deliverables and the user-confirmed target identification (per the brief's "do not mark H2.S1 complete unless `cbm-loop-status --scope broad-goal` passes" caution — left as "current; plan prepared" not "complete" because post-commit broad-goal loop-status has not yet run at the time of this commit).
  - Updated `.planning/STATE.md`: bumped `Last updated:` to 2026-05-22; updated the H2.S1 brief-drafted Summary bullet to record `/goal` execution + locked target + deliverables; added Implementation phase bundles section under Phase Status (Phase 01 closed; Phase 02 active with H2.S1 deliverables produced); added Recent Checkpoints entries for `0819bcd` (pre-built candidates) and this H2.S1 commit; updated pending next work line from H2.S1 to H2.S2 against h11; appended verification footer with templated placeholders for post-commit broad-goal and recovery-slice loop-status outputs (expected `status: ok`, no issues, no warnings, per the brief's two-commit pattern at GOAL-doc:365-394).
  - This BUILD-LOG slice entry (next).
- Tests: not run as part of this iteration. Justification: H2.S1 is planning-doc-only (no source-code change); `TMPDIR=/var/tmp pytest -q` expected to preserve the H1 final count of `146 passed, 2 warnings`. Pre-commit verification recorded below.
- Verification:
  - Pre-commit `TMPDIR=/var/tmp pytest -q` — see Commit 1 verification record (recorded after the commit-staging step).
  - Pre-commit `git diff --check -- .planning BUILD-LOG.md` — see Commit 1 verification record.
  - Post-commit `python3 -m cbm.cli loop-status --repo . --scope broad-goal --work-category runtime-producer --json` — see Commit 1 verification record (templated placeholder: expected `status: ok`, no issues, no warnings; actual JSON shape filled by audit-only Commit 2 if it diverges from the placeholder).
  - Post-commit `python3 -m cbm.cli loop-status --repo . --scope recovery-slice --work-category runtime-producer --json` — see Commit 1 verification record (templated placeholder: expected `status: ok`, no issues, no warnings; actual JSON shape filled by audit-only Commit 2 if it diverges).
  - `pass-claim` loop-status is intentionally NOT part of this slice per the brief's H2.S1 verification scope (currently green on H1 evidence; no H2 information; H2.A5 is gated by the H2.S3 checkpoint).
  - Probe verification commands (read-only, recorded in `H2-TARGET-SELECTION.md` "Locked Pick" + `H2-PLAN.md` "Probe re-verification on 2026-05-22"): all returned the expected SHA + LOC count + MIT license. No Surface Mapper, Skeptic, or live producer invoked.
  - Schema-drift verification: `git log 76db3bc..HEAD --oneline -- schemas cbm/schemas` and `git diff --stat 76db3bc..HEAD -- schemas cbm/schemas` both returned empty output (no commits, no diff bytes) — schemas unchanged since H1 merge.
- Boundary:
  - H2.S1 planning-only slice. No live Surface Mapper or Skeptic invocation against the new target.
  - No H2 completion claim. No repeatability claim. No Phase B+ claim. No beta-readiness claim. H1 acceptance preserved exactly as merged in `76db3bc`.
  - `VISION.md` and `RUNTIME-CONSTITUTION.md` not edited.
  - The post-goal follow-up per the brief's "Post-Goal Follow-Up" section is to open the H2.S2 `/goal` against a new `GOAL-H2S2-LIVE-RUN.md`; that authoring is out of scope for this slice.
  - The Codex CLI runtime cost actuals will be re-measured at H2.S2 dispatch; H2.A4 records the H1/H2 diff in `.planning/STATE.md` at H2.S2 time (not in this H2.S1 slice).
