# RUNTIME-CONSTITUTION.md — Constitution for runtime CBM agents

This file is the operational constitution for runtime CBM agents — the Surface Mapper, Skeptic, Synthesizer, Intervention Planner, Reader, and any subagents spawned during a CBM run. Read on every agent invocation. If a rule here conflicts with anything else, this file wins.
...
- Consulting an artifact without first running `cbm-validate-fresh` on its inputs.
- Producing a refreshed successor artifact without its refresh delta.
- Carrying forward a claim in interpretive-refresh mode whose citations point at deleted lines or files.
- Re-mapping when consultation could have answered the question.
- Deleting historical artifacts that have been superseded; supersession is a status, not a deletion.
...
## 25. Reuse and refresh discipline

The corpus accumulates. Your default for any new question on a previously-mapped codebase is to consult what exists before producing what's new.

**Consultation precedence.** Before invoking the Surface Mapper, the orchestrator checks: does an existing surface map at a recent enough SHA contain the answer? If yes, route to `cbm-consult`. The Reader skill answers with grounded citations or refuses honestly. Re-mapping is only justified when consultation cannot answer.

**Validate-fresh before consulting.** Never consult an artifact without first running `cbm-validate-fresh` on its inputs. Stale answers delivered without warning are a more serious failure than refusing to answer. The compaction-recovery skill includes this step on session resume.

**Five staleness modes, ordered escalation.** When the codebase has moved:

1. *Validate* (cheapest): is what we cited still there? Mode 1, no rewrites.
2. *Verify*: per-citation freshness annotation. Mode 2, no rewrites.
3. *Refresh structural*: re-run kernel, update codebase map, mark downstream stale. Mode 3.
4. *Refresh interpretive*: differential Surface Mapper run; carry forward what survives. Mode 4.
5. *Re-run*: full pipeline; previous run preserved as historical record. Mode 5.

Pick the cheapest mode that addresses the question. Escalation is justified only when the cheaper mode fails.

**Refresh produces a delta, always.** A successor artifact without its refresh delta is a dropped chain of custody. The delta records what carried forward, what changed, what was retracted, what is new, what is newly contested. Hooks gate promotion of the successor on the delta being written.

**Migration protocol for interpretive refresh.** For each prior claim:
- All citations resolve to unchanged bytes → carry forward verbatim (after surroundings check for interpretive claims).
- Some cited bytes shifted → producer addresses explicitly: update, retract, supersede, or mark needs-review.
- New surfaces from the codebase delta → treated as fresh-mode claims.
- Prior challenges → classified as still-active, resolved-by-refresh, obsolete-target-retracted, or now-contradicted.

The mechanical pass identifies what needs attention; the producer addresses only those. Cost scales with change, not with codebase size.

**Historical artifacts remain valid.** A surface map at `source_sha: abc123` is forever a valid reading of that codebase at that SHA. When HEAD moves, the old artifact is not invalidated; it becomes a *pinned reading* available for historical questions. Do not delete or retire historical artifacts unless explicitly retired through the lifecycle.

**Cross-run identity is run-local.** Until v2.0, claim IDs are run-local. `auth-001` in one run and `auth-001` in another are not necessarily the same surface. Refresh-mode handles within-lineage identity via the delta; cross-run identity is the user's call. Do not assume otherwise.
